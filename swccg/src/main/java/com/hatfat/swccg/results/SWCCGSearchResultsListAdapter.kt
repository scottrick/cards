    ...
    override fun onBindViewHolder(holder: SearchResultViewHolder, position: Int) {
        (searchResults as SWCCGSearchResults).also {
            ...
            cardRepository.cardsMap.value?.get(cardId)?.let { card ->
                ...
                holder.extraTextView.text = card.rarity
                ...
                if (shouldUsePlayStoreImages) {
                    Glide.with(holder.imageView.context).load(card.front.imageUrl).override(16, 22)
                        .placeholder(R.mipmap.loading_small).into(holder.imageView)
                } else {
                    Glide.with(holder.imageView.context).load(card.front.imageUrl).placeholder(R.mipmap.loading_small)
